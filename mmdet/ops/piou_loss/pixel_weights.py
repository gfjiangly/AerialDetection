...
def test_pious_fast():
    ...
    corners_p = rbox2corners_torch(loc_p)
    corners_t = rbox2corners_torch(loc_t)
    grid = jaccard_fast(corners_box_form(corners_p),
                        corners_box_form(corners_t))
...
def test_pious():
    for item in range(100):
        ...
        loc_pa = torch.randint(-180, 180, [10, 1]).float() / 180.0 * 3.141593
        loc_p = torch.cat((loc_pxy, loc_pwh, loc_pa), -1)

        loc_txy = torch.randint(10, 300, [10, 2]).float()
        loc_twh = torch.randint(20, 200, [10, 2]).float()
        loc_ta = torch.randint(-180, 180, [10, 1]).float() / 180.0 * 3.141593
        loc_t = torch.cat((loc_txy, loc_twh, loc_ta), -1)
        ...
        if torch.cuda.is_available():
            ...
            print('gap grad piou: ', torch.sum(
                grad_loc_p_big.cuda() - loc_p_grad) / 10)
...
def test_corners():
    ...
    for item in range(3):
        point = cv2.boxPoints(
            ((rbox[item][0], rbox[item][1]), (rbox[item][2], rbox[item][3]), rbox[item][4]))
        print('point: ', point)
...
def test_resize():
    ...
    for item in range(3):
        ...
        rbox = [rect[0][0], rect[0][1], rwidth,
                rheight, rangle / 180.0 * 3.141593]
        print('rbox: ', rbox)
