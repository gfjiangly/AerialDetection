...
class CustomDataset(Dataset):
    ...
    def prepare_train_img(self, idx):
        ...
        if self.rotate_aug is not None:
            # import cvtools
            # cvtools.imwrite(img, '/code/debug/AerialDetection/retinanet/rotate/' + img_info['file_name'])
            ...
            # gt_polys = get_best_begin_point(mask2poly(gt_masks))
            # img_draw = cvtools.draw_boxes_texts(img, gt_polys, box_format='polygon')
            # cvtools.imwrite(img_draw, '/code/debug/AerialDetection/retinanet/rotate/rotate_' + img_info['file_name'])
            # skip the image if there is no valid gt bbox
            if len(gt_bboxes) == 0:
                return None
        # apply transforms
